...
{
	if (less_than_32b(tsk->snd_wnd - tsk->snd_nxt + tsk->snd_una, TCP_MSS))
	{
		return 0; // 窗口足够
	}
	return 1; // 窗口不足
...
struct tcp_sock *alloc_tcp_sock()
{
	...
	init_list_head(&tsk->send_buf);	   // 初始化 send_buf 链表
	init_list_head(&tsk->rcv_ofo_buf); // 初始化 rcv_ofo_buf 链表
	...
	// tsk->wait_wnd = alloc_wait_struct();
...
int tcp_hash(struct tcp_sock *tsk)
{
	...
	tsk->ref_cnt += 1;
	// log(DEBUG, "de10 tcp_hash finish");
	return 0;
}
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	{
		log(ERROR, " to set source port.");
		return -1;
	}
	// 使用路由查找设置源 IP 地址
	rt_entry_t *route = longest_prefix_match(ntohl(skaddr->ip));
	...
	tsk->sk_sip = route->iface->ip;

	// 设置目标 IP 和端口（网络字节序）
	tsk->sk_dip = ntohl(skaddr->ip);
	tsk->sk_dport = ntohs(skaddr->port);

	// 将 TCP 套接字加入 bind_table
	tcp_bind_hash(tsk);
...
int tcp_sock_listen(struct tcp_sock *tsk, int backlog) // 将服务器设置为监听状态
{
	// log(DEBUG, "listen");
	//  fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
	// log(DEBUG, "de2");
	...
	// log(DEBUG, "de2.1");
	tsk->backlog = backlog;
	int hash = tcp_hash_function(0, 0, tsk->sk_sport, 0);
	list_add_head(&tsk->hash_list, &tcp_listen_sock_table[hash]);
	if (tcp_hash(tsk) >= 0)
		return 0;
	return -1;
}
...
{
	// fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
	// log(DEBUG, "close one sock");
	switch (tsk->state)
	{
	case TCP_ESTABLISHED: // 连接状态下发送fin，自己进入wait1
		tcp_send_control_packet(tsk, TCP_FIN | TCP_ACK);
		// log(DEBUG, "de5");
		tcp_set_state(tsk, TCP_FIN_WAIT_1);
		break;

	case TCP_CLOSE_WAIT: // 关闭等待状态下，发送fin，进入last_ack
...
{
	// log(DEBUG, "TCP_SOCK_READ with length %d", len);
	...
	{
		// log(DEBUG, "circle begin");
		int avail = min(len - read_bytes, ring_buffer_used(tsk->rcv_buf));

		if (avail > 0)
		{
			log(DEBUG, "len-read_bytes=%d,buffer_len=%d", len - read_bytes, ring_buffer_used(tsk->rcv_buf));

			read_ring_buffer(tsk->rcv_buf, buf + read_bytes, avail);
			read_bytes += avail;
			tsk->rcv_wnd += avail;

			break;
		}
		if (avail == 0 && tsk->state == TCP_CLOSE_WAIT)
		{
			return -1;
		}
	}
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	// if (len == 0)
	//{
	// char *str = malloc(ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE);

	// tcp_send_packet(tsk, str, ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE);
	// pthread_mutex_unlock(&tsk->sk_lock);

	// return 0;
	//}

	//   log(DEBUG, "TCP_SOCK_WRITE with length %d", len);
	if (tsk->state != TCP_ESTABLISHED && tsk->state != TCP_CLOSE_WAIT)
	{
		return -1;
	}

	// log(DEBUG, "first snd_wnd is %d", tsk->snd_wnd);
	// int mss = min(tsk->snd_wnd, 1460);
	int sent_len = 0;
	// tsk->snd_wnd = 1460;
	...
	while (sent_len < len)
	{
		...
		{
			// log(DEBUG, "sleep and wait for send again");
			// pthread_mutex_unlock(&tsk->sk_lock);
			log(DEBUG, "sleep on wnd");
			sleep_on(tsk->wait_send);
			// pthread_mutex_lock(&tsk->sk_lock);
		}
		int send_len = min(len - sent_len, min(tsk->snd_wnd, TCP_MSS));
		int packet_len = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + send_len;
		char *strs = malloc(sizeof(char) * packet_len + 1);
		memcpy(strs + ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE, buf + sent_len, send_len);

		// pthread_mutex_lock(&tsk->sk_lock);
		//  tcp_send_packet(tsk, buf + sent_len, send_len);
		log(DEBUG, "send seq=%u,ack=%u", tsk->snd_nxt, tsk->rcv_nxt);
		pthread_mutex_lock(&tsk->sk_lock);
		tcp_send_buffer_add_packet(tsk, buf, len);
		tcp_send_packet(tsk, strs, packet_len);
		pthread_mutex_unlock(&tsk->sk_lock);
		// pthread_mutex_unlock(&tsk->sk_lock);

		// log(DEBUG, "send end, relaxing...");
		sent_len += send_len;
	}

	// log(DEBUG, "send data successfully");
	return sent_len;
}
