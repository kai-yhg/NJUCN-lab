...
void iface_send_packet(iface_info_t *iface, const char *packet, int len)
{
	...
	// log(DEBUG, "Trying to send packet: dst_mac = " ETHER_STRING ", iface = %s, fd = %d, index = %d",
	// ETHER_FMT(eh->ether_dhost), iface->name, iface->fd, iface->index);

	ssize_t ret = sendto(iface->fd, packet, len, 0, (const struct sockaddr *)&addr, sizeof(addr));
	if (ret < 0)
	{
